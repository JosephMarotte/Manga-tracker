...
class BaseMangaSiteDatabaseQueryDatabaseQuery:
    ...
    def select_mangatracker_manga_id_from_base_manga_site_manga_id(cls, base_manga_site_manga_id, cursor):
        ...
        logging.info("Checking whether {base_manga_site} manga %s is in our database".format(base_manga_site=cls.base_manga_site) % base_manga_site_manga_id)
        ...
        if mangatracker_manga_id is None:
            logging.info("{base_manga_site} manga %s is not in our database".format(base_manga_site=cls.base_manga_site) % base_manga_site_manga_id)
        else:
            mangatracker_manga_id = mangatracker_manga_id['mangatracker_manga_id']
            logging.info("{base_manga_site} manga %s is in our database with manga id %d".format(base_manga_site=cls.base_manga_site)
    ...
    @classmethod
    def insert_mangatracker_manga_id_to_base_manga_site_manga_id(cls, mangatracker_manga_id, base_manga_site_manga_id, cursor):
        query_tuple = str(mangatracker_manga_id), str(base_manga_site_manga_id)
        logging.info("Adding matching between mangatracker_manga_id %s and {base_manga_site}_manga_id %s".format(base_manga_site=cls.base_manga_site) % query_tuple)
        sql_query = cls.insert_mangatracker_manga_id_to_base_manga_site_manga_id_sql_query()
        cursor.execute(sql_query, query_tuple)
        connection.commit()
        logging.info("Matching between mangatracker_manga_id %s and {base_manga_site}s_manga_id %s was added".format(base_manga_site=cls.base_manga_site) % query_tuple)
